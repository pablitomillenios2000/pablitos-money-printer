...
# 6a. Filter to keep timestamps where Acceleration >= +1.5
df_up = df.loc[df["Acceleration"] >= 1.5, ["Timestamp", "LSMA"]]

# 6b. Filter to keep timestamps where Acceleration <= -1.5
df_down = df.loc[df["Acceleration"] <= -1.5, ["Timestamp", "LSMA"]]
...
print(f"Filtered polyreg values (LSMA) for acceleration >= +1.5 have been saved to {POLYACC_FILE_UP}.")
print(f"Filtered polyreg values (LSMA) for acceleration <= -1.5 have been saved to {POLYACC_FILE_DOWN}.")
